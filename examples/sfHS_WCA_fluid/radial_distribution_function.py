...
class ComputeGR(object):
    def __init__(
        ...
        def volume_nball(radius, n):
            return (
                np.power(np.pi, n / 2) * np.power(radius, n) / gamma(n / 2 + 1)
            )

        self.box_length = np.power(
            np.sum(
                np.asarray(
                    [volume_nball(r, self.boxdim) for r in self.hard_radii]
                )
            )
            / self.hard_phi,
            1 / self.boxdim,
        ...
        self.step = RandomCoordsDisplacement(
            42, 1, single=True, nparticles=self.nr_particles, bdim=self.boxdim
        )
        if self.verbose:
        ...
        self.gr_quench = RecordPairDistHistogram(
            self.box_vector,
            50,
            self.eq_steps,
            self.nr_particles,
        ...
        self.gr = RecordPairDistHistogram(
            self.box_vector, 50, self.eq_steps, self.nr_particles
        )

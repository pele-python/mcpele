...
void Histogram::add_entry(double E) {
  m_moments(E);
  int i;
  E = E + m_eps;  // this is a dirty hack, not entirely sure of its generality
                  // and possible consequences, tests seem to be fine
...
/*
 * Note: This gives the error bar on a bin of width _bin, under the assumption
 * that the sum of all bin areas is 1.
 * */
std::vector<double> Histogram::get_vecdata_error() const {
  std::vector<double> result(m_hist.size(), 0);
  for (size_t i = 0; i < result.size(); ++i) {
    const double this_fraction =
        static_cast<double>(m_hist.at(i)) / static_cast<double>(get_count());
    result.at(i) =
        sqrt(this_fraction * (1 - this_fraction) / m_bin) / sqrt(get_count());
  }
  return result;
}
...
std::vector<double> Histogram::get_vecdata_normalized() const {
  ...
  for (size_t i = 0; i < m_hist.size(); ++i) {
    const double this_fraction =
        static_cast<double>(m_hist.at(i)) / static_cast<double>(m_niter);

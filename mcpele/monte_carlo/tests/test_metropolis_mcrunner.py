...
class TestMetropolisGlobal(unittest.TestCase):
    def setUp(self):
        ...
        self.mcrunner = Metropolis_MCrunner(
            self.potential,
            self.origin,
            1,
            self.stepsize,
    ...
    def test_set_control(self):
        ...
        self.mcrunner.run()
        self.assertEqual(
            self.mcrunner.get_temperature(), self.mcrunner.temperature
        )
    ...
    def test_dump_histogram(self):
        ...
        self.assertListEqual(
            np.ndarray.tolist(binenergy), np.ndarray.tolist(data[:, 0])
        )
        self.assertListEqual(
            np.ndarray.tolist(hist), np.ndarray.tolist(data[:, 1])
        )

...
TEST_F(TestMC, BasicFunctionalityPolyHarmonic){
    ...
    std::shared_ptr<mcpele::TakeStep> sampler_uniform_adaptive = std::make_shared<mcpele::AdaptiveTakeStep>(std::shared_ptr<mcpele::TakeStep>(new mcpele::RandomCoordsDisplacementAllAdaptive(42)), 50);
    EXPECT_TRUE( !mc->take_step_specified() );
...
TEST_F(TestMCMock, AdaptiveTakeStep_WorksDown){
    ...
    mcpele::RandomCoordsDisplacementAllAdaptive* rs = new  mcpele::RandomCoordsDisplacementAllAdaptive(42, 1);
    auto astep = std::make_shared<mcpele::AdaptiveTakeStep>(std::shared_ptr<mcpele::RandomCoordsDisplacementAllAdaptive>(rs), 100, 0.8);

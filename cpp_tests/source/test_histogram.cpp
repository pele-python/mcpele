...
TEST_F(TestHistogram, TestMomentsGlobalMoves) {
  ...
  EXPECT_NEAR_RELATIVE(hist_uniform_all.get_mean(),
                       sampler_uniform_all.expected_mean(),
                       2 * ss / sqrt(ntot));
  EXPECT_NEAR_RELATIVE(hist_gaussian.get_mean(),
                       sampler_gaussian.expected_mean(), 2 * ss / sqrt(ntot));
  EXPECT_NEAR_RELATIVE(hist_uniform_all.get_variance(),
                       sampler_uniform_all.expected_variance(ss),
                       2 * ss / sqrt(ntot));
  EXPECT_NEAR_RELATIVE(hist_gaussian.get_variance(),
                       sampler_gaussian.expected_variance(ss),
                       2 * ss / sqrt(ntot));
}
...
TEST_F(TestHistogram, TestMomentsSingleMoves) {
  ...
  for (size_t step = 0; step < nsteps * 500; ++step) {
    std::fill(displ_uniform_single.data(), displ_uniform_single.data() + ndof,
              0);
  ...
  EXPECT_NEAR_RELATIVE(hist_uniform_single.get_mean(),
                       sampler_uniform_single.expected_mean(),
                       2 * ss / sqrt(ntot));
  EXPECT_NEAR_RELATIVE(hist_uniform_single.get_variance(),
                       sampler_uniform_single.expected_variance(ss),
                       2 * ss / sqrt(ntot));
}
...
TEST_F(TestHistogram, TestBinning) {
  ...
  EXPECT_DOUBLE_EQ(std::accumulate(hist.begin(), hist.end(), double(0)),
                   hist.get_count());
  const std::vector<double> vecdata = hist.get_vecdata();
  EXPECT_EQ(vecdata.size(), hist.size());
  EXPECT_DOUBLE_EQ(std::accumulate(vecdata.begin(), vecdata.end(), double(0)),
                   hist.get_count());
  const std::vector<double> vecdata_normalized = hist.get_vecdata_normalized();
  EXPECT_EQ(vecdata_normalized.size(), hist.size());
  EXPECT_DOUBLE_EQ(std::accumulate(vecdata_normalized.begin(),
                                   vecdata_normalized.end(), double(0)) *
                       bin,
                   1);
  const std::vector<double> error = hist.get_vecdata_error();
  for (size_t ii = 0; ii < hist.size(); ++ii) {
    const double xi = hist.get_position(ii);
    const double true_i = exp(-pow((xi - sampler.expected_mean()), 2) /
                              (2 * sampler.expected_variance(ss))) /
                          sqrt(M_PI * 2 * sampler.expected_variance(ss));
